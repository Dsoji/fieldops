  ...
  return (
    <header className="sticky top-0 z-10 flex h-14 items-center gap-3 border-b border-border bg-bg/85 px-4 backdrop-blur md:px-8">
      <div className="flex items-center gap-2 md:hidden">
        <div className="grid size-7 place-items-center rounded bg-accent font-mono text-xs font-bold text-sidebar">F</div>
        <span className="text-sm font-semibold">FieldOps</span>
      </div>
      ...
      <label className="hidden h-9 max-w-sm flex-1 items-center gap-2 rounded-md border border-border bg-surface px-3 text-sm text-faint md:flex">
        ...
        <input
          placeholder="Search assets, sites, work orders…"
      ...
      <div className="ml-auto flex items-center gap-2">
        <Link
          ...
          className="inline-flex h-9 items-center gap-1.5 rounded-md bg-text px-3 text-sm font-medium text-white hover:bg-text/90"
        >
        ...
        <div className="ml-1 flex items-center gap-2">
          <div className="grid size-9 place-items-center rounded-full bg-accent/20 text-xs font-semibold text-accent-ink">
            {initials(currentUser.fullName)}
          </div>
          <div className="hidden leading-tight lg:block">
            <div className="text-[13px] font-medium">{currentUser.fullName}</div>
            <div className="text-[11px] text-muted">{currentUser.title}</div>
          </div>
        </div>
      </div>
    </header>
